...
/**
 * 添加追番记录请求 DTO，只暴露可填写的字段，userId 由 JWT 获取。
 */
@Data
@Schema(description = "添加追番记录请求体")
public class WatchLogAddDTO {
    ...
    @Schema(description = "结束集数", example = "1")
    private Integer epEnd;

    @Schema(description = "观看集数，不填默认为 1", example = "1")
    private Integer epCount;

    @Schema(description = "观看集号明细，逗号分隔（含SP/OVA/OAD番外）", example = "1,2,3,SP")
    private String epNos;

    @Schema(description = "备注", example = "一口气看完，超爽")
    private String notes;

    /**
     * 将 DTO 转换为 WatchLog 实体对象（不含 userId，由调用方设置）
     */
